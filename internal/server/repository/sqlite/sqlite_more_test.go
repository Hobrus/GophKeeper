...
func TestGetUserByEmail_NoRows(t *testing.T) {
	repo, _ := New("file:repo_no_user?mode=memory&cache=shared&_journal=WAL")
...
func TestDeleteRecord_NoRows(t *testing.T) {
	repo, _ := New("file:repo_delete_no_rows?mode=memory&cache=shared&_journal=WAL")
	t.Cleanup(func() { _ = repo.Close() })
...
func TestListRecords_MetaDecode(t *testing.T) {
	repo, _ := New("file:repo_list_meta?mode=memory&cache=shared&_journal=WAL")
	t.Cleanup(func() { _ = repo.Close() })
	ctx := context.Background()
...
func TestMigrations_Idempotent(t *testing.T) {
	repo, err := New("file:repo_migrations?mode=memory&cache=shared&_journal=WAL")
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = repo.Close() })

	// Повторное создание репозитория над тем же DSN не должно падать,
	// так как миграции идемпотентны и записываются в schema_migrations.
	repo2, err := New("file:repo_migrations?mode=memory&cache=shared&_journal=WAL")
	if err != nil {
